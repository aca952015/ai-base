...
const GATEWAY_CONFIG_VERSION_MARKER = "# ai-base-gateway-config-version: 2";
const CUSTOM_MCP_ADAPTER_HOST = "mcp-backend-adapter";
const CUSTOM_MCP_ADAPTER_PORT = 8090;
const CUSTOM_MCP_ADAPTER_PATH_PREFIX = "/internal/v1/mcp-backends";
...
const PUBLIC_MCP_TOOL_NAMESPACES = new Set(["connector", "kb"]);
...
export function validateGatewayMcpServersInput(input: unknown): GatewayMcpServersValidation {
  ...
  for (const [index, server] of servers.entries()) {
    ...
    if (PUBLIC_MCP_TOOL_NAMESPACES.has(server.namespace) || server.namespace.startsWith("mcp-")) {
      errors.push(`servers[${index}].namespace is reserved for the public MCP tool proxy`);
    }
...
`;
  ...
  const mcpBackendRefs = enabledMcpServers.map((server) => {
    ...
    const backendPath = server.managed
      ? endpoint.pathname
      : `${CUSTOM_MCP_ADAPTER_PATH_PREFIX}/${server.namespace}`;
    ...
    const toolExcludes = server.toolExcludes.length > 0
      ...
      : "";
    const securityPolicy = server.managed && server.keyConfigured
...
` : "";
  ...
  const mcpBackends = enabledMcpServers.map((server) => {
    const name = mcpBackendName(server.namespace);
    const endpoint = new URL(server.url);
    const hostname = server.managed ? endpoint.hostname : CUSTOM_MCP_ADAPTER_HOST;
    const port = server.managed
      ? Number(endpoint.port || (endpoint.protocol === "https:" ? 443 : 80))
      : CUSTOM_MCP_ADAPTER_PORT;
    const endpointBlock = isIP(hostname)
      ? `ip:\n        address: ${yamlString(hostname)}`
      : `fqdn:\n        hostname: ${yamlString(hostname)}`;
    const tlsPolicy = server.managed && endpoint.protocol === "https:" ? `apiVersion: gateway.networking.k8s.io/v1alpha3
kind: BackendTLSPolicy
metadata:
  name: ${name}-tls
  namespace: default
...
async function ensureSystemManagedMcpRegistration(snapshot: GatewayMcpServersSnapshot) {
  ...
  await chmod(paths.mcpServers, 0o644).catch((error: NodeJS.ErrnoException) => {
  ...
  const staleGeneratedConfig = currentConfig !== "" && !currentConfig.includes(GATEWAY_CONFIG_VERSION_MARKER);
  if (!missingManagedBackend && !staleGeneratedConfig && !(await systemManagedMcpSecretsNeedSync())) return;
